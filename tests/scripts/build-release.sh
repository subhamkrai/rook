...
function publish() {
    ...
    git status &
    git diff &
    $MAKE -C build/release publish BRANCH_NAME=${BRANCH_NAME} TAG_WITH_SUFFIX=${TAG_WITH_SUFFIX} GIT_API_TOKEN=${GIT_API_TOKEN}
}
...
export DOCS_GIT_REPO="${DOCS_GIT_REPO:-git@github.com:subhamkrai/rook.github.io.git}"
