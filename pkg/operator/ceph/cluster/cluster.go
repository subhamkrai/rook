...
func (c *cluster) preMonStartupActions(cephVersion cephver.CephVersion) error {
	// Reconcile network policies for Ceph daemons early so the cluster won't be
	// created in a broken state if policies fail to apply.
...
func (c *cluster) postMonStartupActions(imageCephVersion cephver.CephVersion) error {
	...
	if c.Spec.IsStretchCluster() {
		log.NamespacedDebug(c.Namespace, logger, "stretch cluster detected, muting MON_NETSPLIT health warning")
		err := client.MuteHealthWarning(c.context, c.ClusterInfo, "MON_NETSPLIT", "mute")
		if err != nil {
			log.NamespacedError(c.Namespace, logger, "failed to mute MON_NETSPLIT health warning: %v", err)
		}
	}
