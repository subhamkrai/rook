...
func (c *ClusterController) configureExternalCephCluster(cluster *cluster) error {
	...
	// We no longer create the CSI secrets for external clusters in downstream, so disable this for external clusters.
	if cluster.ClusterInfo.CephCred.Username == client.AdminUsername && false {
...
func validateExternalClusterSpec(cluster *cluster) error {
	if cluster.Spec.Monitoring.MetricsTLS.SecretName != "" {
